...
namespace Lrt_Ilukste
{
    public partial class FormOper : Form
    {
        ...
        private BindingList<SaabOPCToDataGrid> dgwList = new BindingList<SaabOPCToDataGrid>();
        ...
        private void FormOper_Load(object sender, EventArgs e)
        {
            ...
            // Format DataGridView
            dataGridView1.Columns[0].HeaderText = "Дата/Время";
            ...
            dataGridView1.Columns[3].DefaultCellStyle.Format = "F1";
            ...
            dataGridView1.Columns[5].DefaultCellStyle.Format = "F3";
            dataGridView1.Columns[5].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            dataGridView1.Columns[6].HeaderText = "Мi,т";
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (autoRead == true)
            {
                if (dataSource == true)     // Data from OPC Server
                {
                    buttonSaabData.Visible = false;
                    labelSaabRead.Text = "Считывание данных с OPC сервера: АВТОМАТИЧЕСКОЕ через";
                    textBoxSaabReadCounter.Visible = true;
                    ...
                    if (SaabAutoReadCounter == 0)
                    {
                        ...
                        //FormMain.OPCDataReady
                        //if(FormMain.OPCDataReady)
                        //{

                        if (FormMain.dgwOPCList.Count == 16)
                        {
                            List<SaabOPCToDataGrid> ls = FormMain.dgwOPCList.ToList();
                            dgwList.Clear();
                            //dgwList = (BindingList<SaabOPCToDataGrid>)ObjectCloner.Clone(obj: FormMain.dgwOPCList);
                            System.Collections.IList list1 = ls;
                            for (int i = 0; i < list1.Count; i++)
                            {
                                //List<SaabOPCToDataGrid> list = (List<SaabOPCToDataGrid>)list1[i];
                                dgwList.Add((SaabOPCToDataGrid)list1[i]);
                            }
                            dataGridView1.Refresh();
                        }

                        SaabAutoReadCounter = Properties.Settings.Default.SaabTimeInterval;
                        textBoxSaabReadCounter.Text = "0";
                    }
                    else
                    {
                        textBoxSaabReadCounter.Text = SaabAutoReadCounter.ToString();
                        SaabAutoReadCounter--;
                    }
                }
                else
                {
                    buttonSaabData.Visible = false;
                    labelSaabRead.Text = "Считывание из базы данных: АВТОМАТИЧЕСКОЕ через";
                    textBoxSaabReadCounter.Visible = true;
                    labelSeconds.Visible = true;
                    if (SaabAutoReadCounter == 0)
                    {
            ...
            else
            {
                if (dataSource == true)     // Data from OPC Server
                {
                    buttonSaabData.Visible = true;
                    labelSaabRead.Text = "Считывание данных с OPC сервера: РУЧНОЕ";
                    textBoxSaabReadCounter.Visible = false;
                    labelSeconds.Visible = false;
                }
                else
                {
                    buttonSaabData.Visible = true;
                    labelSaabRead.Text = "Считывание из базы данных: РУЧНОЕ";
                    textBoxSaabReadCounter.Visible = false;
                    labelSeconds.Visible = false;
        ...
        private void ReadDataFromDB()
        {
            ...
            {
                var tank = db.Tanks.OrderBy(x=>x.TankID);
                foreach (Tanks t in tank)
                {
                    //Where(x=>x.TankID == t.TankID)
                    var part = db.SaabData.Where(x => x.TankID == t.TankID).OrderByDescending(x => x.SaabDateTime).Take(1).ToList();
                    for (int i = 0; i < part.Count; i++)
                    {
                        SaabOPCToDataGrid sd = new SaabOPCToDataGrid();
                        sd.TankDateTime = (DateTime)part[i].SaabDateTime;
                        sd.TankName = "N" + part[i].TankID.ToString();
                        sd.TankAvgTemp = (float)part[i].TankAvgTemp;
                        sd.TankDensity = (float)part[i].TankDensity;
                        sd.TankLevel = (float)part[i].TankLevel;
                        sd.TankVolume = (float)part[i].TankVolume;
                        sd.TankMassa = (float)part[i].TankMassa;
                ...
                //var part = db.SaabData.Select<>
                //var part = db.SaabData.OrderByDescending(x => x.SaabDateTime).ThenBy(x => x.TankID).Take(16).ToList();
                //for (int i = 0; i < part.Count; i++)
                //{
                //    SaabOPCToDataGrid sd = new SaabOPCToDataGrid();
                //    sd.TankDateTime = (DateTime)part[i].SaabDateTime;
        ...
        private void ReadDataFromOPCServer()
        {
            ...
            using (var server = new OpcDaServer(url))
            {
                ...
                dgwList.Add(opc1);
                SaveSaabDataToDB(dt, 1, opc1.TankLevel, opc1.TankAvgTemp, td1, tv1, opc1.TankMassa);
               ...
               SaabOPCToDataGrid opc2 = new SaabOPCToDataGrid();
                ...
                dgwList.Add(opc2);
                SaveSaabDataToDB(dt, 2, opc2.TankLevel, opc2.TankAvgTemp, td2, tv2, opc2.TankMassa);
                ...
                dgwList.Add(opc3);
                SaveSaabDataToDB(dt, 3, opc3.TankLevel, opc3.TankAvgTemp, td3, tv3, opc3.TankMassa);
                ...
                dgwList.Add(opc4);
                SaveSaabDataToDB(dt, 4, opc4.TankLevel, opc4.TankAvgTemp, td4, tv4, opc4.TankMassa);
                ...
                dgwList.Add(opc5);
                SaveSaabDataToDB(dt, 5, opc5.TankLevel, opc5.TankAvgTemp, td5, tv5, opc5.TankMassa);
                ...
                dgwList.Add(opc6);
                SaveSaabDataToDB(dt, 6, opc6.TankLevel, opc6.TankAvgTemp, td6, tv6, opc6.TankMassa);
                ...
                dgwList.Add(opc7);
                SaveSaabDataToDB(dt, 7, opc7.TankLevel, opc7.TankAvgTemp, td7, tv7, opc7.TankMassa);
                ...
                dgwList.Add(opc8);
                SaveSaabDataToDB(dt, 8, opc8.TankLevel, opc8.TankAvgTemp, td8, tv8, opc8.TankMassa);
                ...
                dgwList.Add(opc9);
                SaveSaabDataToDB(dt, 9, opc9.TankLevel, opc9.TankAvgTemp, td9, tv9, opc9.TankMassa);
                ...
                dgwList.Add(opc10);
                SaveSaabDataToDB(dt, 10, opc10.TankLevel, opc10.TankAvgTemp, td10, tv10, opc10.TankMassa);
                ...
                dgwList.Add(opc11);
                SaveSaabDataToDB(dt, 11, opc11.TankLevel, opc11.TankAvgTemp, td11, tv11, opc11.TankMassa);
                ...
                dgwList.Add(opc12);
                SaveSaabDataToDB(dt, 12, opc12.TankLevel, opc12.TankAvgTemp, td12, tv12, opc12.TankMassa);
                ...
                dgwList.Add(opc13);
                SaveSaabDataToDB(dt, 13, opc13.TankLevel, opc13.TankAvgTemp, td13, tv13, opc13.TankMassa);
                ...
                dgwList.Add(opc14);
                SaveSaabDataToDB(dt, 14, opc14.TankLevel, opc14.TankAvgTemp, td14, tv14, opc14.TankMassa);
                ...
                dgwList.Add(opc15);
                SaveSaabDataToDB(dt, 15, opc15.TankLevel, opc15.TankAvgTemp, td15, tv15, opc15.TankMassa);
                ...
                dgwList.Add(opc16);
                SaveSaabDataToDB(dt, 16, opc16.TankLevel, opc16.TankAvgTemp, td16, tv16, opc16.TankMassa);
        ...
        //private void buttonSaveSaabData_Click(object sender, EventArgs e)
        //{
        //    SaveSaabDataToDB(DateTime.Now, 1, 1234, Convert.ToSingle("21.5"), Convert.ToSingle("856.7"), Convert.ToSingle("7777"), Convert.ToSingle("8888"));
        //}

        private void SaveSaabDataToDB( DateTime SaabDateTime, int TankID, float TankLevel, float TankAvgTemp, float TankDensity, float TankVolume,  float TankMassa)
        {
            using (IluksteEntities db = new IluksteEntities())
            {
                SaabData sb = new SaabData();
                sb.SaabDateTime = SaabDateTime;
                sb.TankID = (int)TankID;
                sb.TankLevel = TankLevel;
                sb.TankAvgTemp = TankAvgTemp;
                sb.TankDensity = TankDensity;
                sb.TankVolume = TankVolume;
                sb.TankMassa = TankMassa;
        ...
        private void buttonSaabData_Click(object sender, EventArgs e)
        {
            bool dataSource = Properties.Settings.Default.SaabDataSource;
            if (dataSource == true)     // Data from OPC Server
            {
